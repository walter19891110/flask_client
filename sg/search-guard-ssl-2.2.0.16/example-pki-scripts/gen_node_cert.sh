...
openssl pkcs12 -in "$NODE_NAME-keystore.p12" -out "$NODE_NAME.all.pem" -nodes -passin "pass:$KS_PASS"
openssl pkcs12 -in "$NODE_NAME-keystore.p12" -out "$NODE_NAME.key.pem" -nocerts -nodes -passin pass:$KS_PASS
openssl pkcs12 -in "$NODE_NAME-keystore.p12" -out "$NODE_NAME.crt.pem" -clcerts -nokeys -passin pass:$KS_PASS
cat "$NODE_NAME.crt.pem" ca/chain-ca.pem  > "$NODE_NAME.crtfull.pem"
